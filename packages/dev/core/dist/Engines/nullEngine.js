...
export class NullEngine extends Engine {
     ...
     */
    createRawTexture(data, width, height, format, generateMipMaps, invertY, samplingMode, compression = null, type = Constants.TEXTURETYPE_UNSIGNED_INT, creationFlags = 0) {
     ...
     * @param type defines the type fo the data (Engine.TEXTURETYPE_UNSIGNED_INT by default)
     */
    updateRawTexture(texture, data, format, invertY, compression = null, type = Constants.TEXTURETYPE_UNSIGNED_INT) {
        if (texture) {
            texture._bufferView = data;
            texture.format = format;
            texture.invertY = invertY;
